...
pub fn read(size: usize) -> crate::Result<Vec<u8>>
{
    ...
    handle_unsafe!(
        crate::rp::rp_UartRead(buffer.as_mut_ptr(), &mut len)
    )?;
...
pub fn write(buffer: &[u8]) -> crate::Result<()>
{
    ...
    handle_unsafe!(
        crate::rp::rp_UartWrite(b.as_mut_ptr(), buffer.len() as i32)
    )
}
...
{
    handle_unsafe!(
        crate::rp::rp_UartSetStopBits(mode)
    )
}
...
{
    handle_unsafe!(
        crate::rp::rp_UartSetParityMode(mode)
    )
}
