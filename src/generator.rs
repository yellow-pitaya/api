...
pub fn out_is_enable(channel: super::Channel) -> crate::Result<bool> {
    ...
    match handle_unsafe!(crate::rp::rp_GenOutIsEnabled(channel, &mut value)) {
        Ok(_) => Ok(value),
        Err(err) => Err(err),
    }
}
...
pub fn amp(channel: super::Channel) -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetAmp(channel, &mut amplitude)) {
...
pub fn offset(channel: super::Channel) -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetOffset(channel, &mut offset)) {
        Ok(_) => Ok(offset * 20.0),
        Err(err) => Err(err),
    }
}
...
pub fn freq(channel: super::Channel) -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetFreq(channel, &mut frequency)) {
        Ok(_) => Ok(frequency),
        Err(err) => Err(err),
    }
}
...
pub fn phase(channel: super::Channel) -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetPhase(channel, &mut phase)) {
        Ok(_) => Ok(phase),
        Err(err) => Err(err),
    }
}
...
pub fn waveform(channel: super::Channel) -> crate::Result<Waveform> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetWaveform(channel, &mut waveform)) {
        Ok(_) => Ok(waveform),
        Err(err) => Err(err),
    }
}
...
pub fn arb_waveform(channel: super::Channel) -> crate::Result<Vec<f32>> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetArbWaveform(
        channel,
        slice.as_mut_ptr(),
        &mut length
    )) {
...
pub fn duty_cycle(channel: super::Channel) -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetDutyCycle(channel, &mut ratio)) {
        Ok(_) => Ok(ratio),
        Err(err) => Err(err),
    }
}
...
pub fn mode(channel: super::Channel) -> crate::Result<Mode> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetMode(channel, &mut mode)) {
        Ok(_) => Ok(mode),
        Err(err) => Err(err),
    }
}
...
pub fn burst_count(channel: super::Channel) -> crate::Result<i32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetBurstCount(channel, &mut num)) {
        Ok(_) => Ok(num),
        Err(err) => Err(err),
    }
}
...
pub fn burst_repetitions(channel: super::Channel) -> crate::Result<i32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetBurstRepetitions(
        channel,
        &mut repetitions
    )) {
        Ok(_) => Ok(repetitions),
...
pub fn burst_period(channel: super::Channel) -> crate::Result<u32> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetBurstPeriod(channel, &mut period)) {
        Ok(_) => Ok(period),
        Err(err) => Err(err),
    }
}
...
pub fn trigger_source(channel: super::Channel) -> crate::Result<TrigSrc> {
    ...
    match handle_unsafe!(crate::rp::rp_GenGetTriggerSource(channel, &mut src)) {
        Ok(_) => Ok(src),
        Err(err) => Err(err),
    }
}
...
pub fn enable_temp_protection(channel: super::Channel) -> crate::Result<bool> {
    ...
    match handle_unsafe!(crate::rp::rp_GetEnableTempProtection(channel, &mut enable)) {
        Ok(_) => Ok(enable),
        Err(err) => Err(err),
    }
}
...
pub fn latch_temp_alarm(channel: super::Channel) -> crate::Result<bool> {
    ...
    match handle_unsafe!(crate::rp::rp_GetLatchTempAlarm(channel, &mut status)) {
        Ok(_) => Ok(status),
        Err(err) => Err(err),
    }
}
...
pub fn runtime_temp_alarm(channel: super::Channel) -> crate::Result<bool> {
    ...
    match handle_unsafe!(crate::rp::rp_GetRuntimeTempAlarm(channel, &mut status)) {
        Ok(_) => Ok(status),
        Err(err) => Err(err),
    }
}
...
pub fn pll_control_enable() -> crate::Result<bool> {
    ...
    match handle_unsafe!(crate::rp::rp_GetPllControlEnable(&mut enable)) {
        Ok(_) => Ok(enable),
        Err(err) => Err(err),
    }
}
...
pub fn pll_control_locked() -> crate::Result<bool> {
    ...
    match handle_unsafe!(crate::rp::rp_GetPllControlLocked(&mut locked)) {
        Ok(_) => Ok(locked),
        Err(err) => Err(err),
    }
}
...
pub fn burst_last_value(channel: super::Channel) -> crate::Result<f32> {
    ...
    handle_unsafe!(crate::rp::rp_GenGetBurstLastValue(channel, &mut amplitude))?;
