...
pub fn source() -> crate::Result<Source> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetTriggerSrc(&mut source)) {
        Ok(_) => Ok(source),
        Err(err) => Err(err),
    }
}
...
pub fn state() -> crate::Result<State> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetTriggerState(&mut state)) {
        Ok(_) => Ok(state),
        Err(err) => Err(err),
    }
}
...
pub fn delay() -> crate::Result<i32> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetTriggerDelay(&mut decimated_data_num)) {
...
pub fn delay_ns() -> crate::Result<i64> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetTriggerDelayNs(&mut time_ns)) {
        Ok(_) => Ok(time_ns),
        Err(err) => Err(err),
    }
}
...
pub fn pre_counter() -> crate::Result<u32> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetPreTriggerCounter(&mut value)) {
        Ok(_) => Ok(value),
        Err(err) => Err(err),
    }
}
...
pub fn level(channel: Channel) -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetTriggerLevel(channel, &mut volatage)) {
        Ok(_) => Ok(volatage),
        Err(err) => Err(err),
    }
}
...
pub fn hysteresis() -> crate::Result<f32> {
    ...
    match handle_unsafe!(crate::rp::rp_AcqGetTriggerHyst(&mut volatage)) {
        Ok(_) => Ok(volatage),
        Err(err) => Err(err),
    }
}
